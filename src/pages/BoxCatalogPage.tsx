...
export default function BoxCatalogPage() {
    ...
    async function run() { setBusy(true); setError(''); setPicked(undefined); setResult(undefined); try {
        const req: BoxCatalogRequest = { units: unit, skus: imported(parseText(skuText)), orders: importHistory(history), ...(current.trim() ? { currentBoxes: JSON.parse(current) } : {}), ...(candidates.trim() ? { candidateBoxes: JSON.parse(candidates) } : {}), catalogSize: size, billing: { unit: billingUnit, dimDivisor: divisor, minBillableWeight: Number(minimum), ...(rate.trim() ? { ratePerKgOrLb: Number(rate) } : {}) }, voidFillCostPerLitre: Number(voidCost) };
    ...
    {result && request && <section className="space-y-4"><h2>{T('Best option found', '搵到嘅最佳方案')}</h2>
      ...
      {result.savings && <p>{T('Savings per 1,000 orders', '每千單節省')}: {result.savings.costPer1000Orders?.toFixed(2) ?? '—'} · {result.savings.billedWeightPct.toFixed(2)}% {T('billed weight', '計費重量')}</p>}
    ...
    <aside className="bg-amber-50 border rounded p-4">{T('Heuristic estimate, not an optimum. No cushioning/dunnage model. Fragile means zero top-load weight; upright preserves height. Divisor and rates are yours: verify with your buyer/carrier. Unfit orders remain counted and charged at the largest candidate for comparison only. No carrier rounding, box tare or outer-wall allowance. Paccurate and Packsize exist for enterprise workflows.', '呢個係啟發式估算，唔保證最優。冇緩衝／墊料模型。易碎表示頂部承重為零；直立會保持高度。除數同費率由你提供，請向買家／承運商核實。放唔落嘅訂單仍會計數，並按最大候選箱估費，只作比較。冇計承運商進位、空箱重量或箱壁厚度。企業流程可考慮 Paccurate 同 Packsize。')}</aside>
  </main>;
}
