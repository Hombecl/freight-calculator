...
export default function BuildSheetPage() {
  ...
  return <main className="[&_input[type=file]]:max-w-full [&_fieldset]:min-w-0 [&_label]:min-w-0 break-words max-w-6xl mx-auto px-4 py-8 space-y-4">
    ...
    <div className="print:hidden space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><h1 className="text-3xl font-black text-slate-900">{T('Crew build sheet','倉務砌板單')}</h1><button className={btnCls} onClick={()=>{window.print();track('build_sheet_print');}}>{T('Print','列印')}</button></div>
      ...
      <label className="block">{T('Receiver profile','收貨方設定')}<select className={inputCls} value={profileId} onChange={e=>{setProfileId(e.target.value);setManual({});}}>{profiles.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label>
      <Link className="underline" to={lang==='zh'?'/zh/order-quote':'/order-quote'}>{T('Return to quote and variance','返回報價同偏差記錄')}</Link></section>
    </div>
    {error&&<p role="alert">{error}</p>}{q&&<><div className="rounded-2xl border-2 border-slate-200 bg-slate-50 p-5 space-y-2"><p className="font-bold text-slate-900"><span className="mr-2 inline-flex rounded-full bg-slate-200 px-2 py-1 text-xs font-black uppercase">{q.status}</span>{q.orderId??q.meter.id} · {q.summary.cartonsPlaced}/{q.summary.cartonsRequested} {T('cartons placed','箱已放入')} · {q.engineVersion}</p><p className="text-xs">{q.semantics}</p></div>
    {profile.template&&<p className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">{T('Verify with your buyer','請同買家核實')}</p>}<p>{profile.name} · {version} · {profile.approvedBy} {profile.approvedAt}</p>{!!profile.unverified?.length&&<p>{T('Unverified','未核實')}: {profile.unverified.join(', ')}</p>}
    {q.pallets.map(p=><article key={q.inputHash+p.index} data-testid="sheet-pallet" className="sheet-pallet rounded-2xl border border-slate-200 p-5 space-y-4">
      <h2 className="text-xl font-black text-slate-900">{T('Pallet','卡板')} {p.index+1} · {p.cartonCount} {T('cartons','箱')}</h2><p className="text-xs">{q.orderId??q.meter.id} · {profile.name} · {T('Profile version','設定版本')}: {version}</p>
