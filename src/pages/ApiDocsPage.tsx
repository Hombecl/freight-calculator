...
curl -X POST https://www.dimpack3d.com/api/order-options -H 'Content-Type: application/json' --data-binary @order-options-request.json`}</code></pre>
      ...
      <section id="box-catalog" className="border rounded p-5 mb-8">
        ...
        <p>{T('Best option found, not optimum. No cushioning, carrier rounding or box tare. Unfit orders are flagged and charged at the largest candidate by volume, including baseline comparison. Box cost and void fill join the monetary objective only when a rate is supplied; otherwise search uses billed kg. Low search budgets fill remaining slots deterministically. chooseBox uses the smallest billed weight for a complete single order.', '只係搵到嘅最佳方案，唔保證最優。冇緩衝、承運商進位或空箱重量。放唔落嘅訂單會標示並按最大體積候選箱估費，基準比較亦一樣。有費率先將箱費同填充費加入目標；否則按計費 kg 搜尋。預算太低時餘下箱型會按固定次序補齊。chooseBox 為完整單張訂單揀最低計費重量紙箱。')}</p>
